import { PrototypeChain } from '../meta'
...
class Callbacks {
  ...
  static set_callback(type, function_name) {
    return this.callback_manager.set_callback(type, function_name)
  }

  static run_callbacks(type, instance) {
    const inheritance = new PrototypeChain(this)
    let i = 0
    while (inheritance[i].callback_manager instanceof CallbackManager) {
      inheritance[i].callback_manager.run_callbacks(type, instance)
      i++
    }
  }
