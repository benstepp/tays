...
class VisitingMethods {
  ...
  visit_AlterTableStatement(node, collector) {
    ...
    this.maybe_visit_many(node.columns, collector, { prefix: ' ADD COLUMN ', delimiter: ', ADD COLUMN' })
  }
  ...
  visit_Column(node, collector) {
    ...
    collector.push(' ')
    this.visit(node.type, collector)
  ...
  visit_NullConstraint(node, collector) {
    if (!node.expression) { collector.push(' NOT') }
    collector.push(' NULL')
  }
