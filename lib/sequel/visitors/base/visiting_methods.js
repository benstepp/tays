...
class VisitingMethods {
  ...
  visit_AlterTableStatement(node, collector) {
    ...
    this.maybe_visit(node.rename, collector)
  ...
  visit_Cascade(node, collector) {
    collector.push('CASCADE')
  }
  ...
  visit_CreateTableStatement(node, collector) {
    ...
    this.maybe_visit(node.temporary, collector)
  ...
  visit_DropTableStatement(node, collector) {
    collector.push('DROP TABLE ')
    this.maybe_visit(node.if_exists, collector, { prefix: '' })
    this.visit(node.table, collector)
    this.maybe_visit(node.cascade, collector)
  }
  ...
  visit_IfExists(node, collector) {
    collector.push('IF EXISTS ')
  }
  ...
  visit_InsertStatement(node, collector) {
    collector.push('INSERT INTO ')
    this.visit(node.relation, collector)
    this.maybe_visit_many(node.columns, collector, { prefix: ' (', suffix: ')' })
    collector.push(' VALUES ')
    this.maybe_visit_many(node.values, collector, { prefix: '(', suffix: ')' })
  }
  ...
  visit_RenameColumn(node, collector) {
    collector.push('RENAME COLUMN ')
    this.visit(node.left, collector)
    collector.push(' TO ')
    this.visit(node.right, collector)
  }

  visit_RenameTable(node, collector) {
    collector.push('RENAME TO ')
    this.visit(node.expression, collector)
  }

  visit_Restrict(node, collector) {
    collector.push('RESTRICT')
  }
  ...
  visit_Temporary(node, collector) {
    collector.push('TEMPORARY')
  }
  ...
  visit_TypeVarchar(node, collector) {
    collector.push('character varying')
    if (node.length) collector.push(`(${node.length})`)
  }

  visit_UniqueConstraint(node, collector) {
    collector.push(' UNIQUE')
  }
